...
describe('scan', function () {
    ...
    before(() => {
        ...
        if (!cellA1Result.isOk)
            panic('cannot parse cell-ref "a1"');
        CELL_A1 = cellA1Result.val;
        const cellE9Result = CellRef.parse('e9');
        if (!cellE9Result.isOk)
            panic('cannot parse cell-ref "e9"');
    ...
    it('multiple tokens should scan correctly', () => {
        ...
        expect(tokens.map(t => t.kind)).to.deep.equal(['num', '+', '(', 'ref', '*', 'ref', ')', 'EOF']);
        expect(tokens[0].value).to.equal(1.23);
