...
async function doAllTests(paths: [string, string][]) {
  for (const [descr, path] of paths) {
    const { default: makeSpreadsheet, Spreadsheet } = await import(path);

    describe(descr, function() {

      let spreadsheet: typeof Spreadsheet;
...
await doAllTests([
  ['undo spreadsheet', '../lib/spreadsheet.js'],
  ['shadow spreadsheet', '../lib/spreadsheet-shadow.js'],
  ['validate spreadsheet', '../lib/spreadsheet-validate.js'],
]);
