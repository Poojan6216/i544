...
describe('web services', () => {
    ...
    beforeEach(async function () {
        ...
        const app = makeApp(ssServices, BASE);
        ws = supertest(app);
    });
    ...
    async function setCells(ssName, updates) {
    ...
    async function queryCells(ssName, tests) {
    ...
    async function loadSS(ssName, dump) {
    ...
    async function readSS(ssName, dump) {
        const url = `${BASE}/${ssName}`;
        const res = await ws.get(url);
        expect(res.status === STATUS.OK);
        expect(res.body?.isOk).to.equal(true);
    ...
    function evalTestToSSDump(evalTestData) {
        return evalTestData
